...
const colorFor = (cat = "") => "bg-green-500";
  ...
  return (
    <div className="w-full">
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-xl p-3 md:p-4 h-full flex flex-col min-h-0">
        ...
        {/* Scrollable content (week header + grid) */}
        <div
          className={`flex-1 min-h-0 ${scroll ? "overflow-y-auto pr-1" : ""}`}
          style={{ scrollbarGutter: "stable" }}
        >
          {/* Week header */}
          <div className="grid grid-cols-7 gap-1 mb-2 sm:mb-3 sticky top-0 bg-transparent">
            {dayNames.map((day) => (
              <div
                key={day}
